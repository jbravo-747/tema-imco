					...
					<a href="<?php echo get_permalink($post_item->ID);?>" class="post_item" >
						<div class="post_item_wrapper">
							<div class="post_item_content clear">
								<div class="post_item_image_container" style="background-image:url(<?php echo $post_item_image[0]; ?>)"></div>
			...
			<div class="filters_container">
					...
					<form class="filters_form" method="get" action="<?php echo get_permalink($page->ID)?>">
						...
						<div class="filter_container post_type_filter <?php echo !empty($_GET['t'])?'open':'';?>">
							<div class="filter_wrapper">
								<div class="filter_header">Tipo</div>
...
<div class="post_author_container">
	<div class="post_author_wrapper">
		<div class="post_author_container_content clear">
			<div class="post_author_image" style="background-image:url(<?php echo $author_image;?>);"></div>
			<div class="post_author_info">
				<div class="post_author_header">
					<label><?php echo $author_name;?></label><?php 
	...
	if(!empty($posts)){
		...
		<div class="home_elections_container">
			<div class="home_elections_wrapper">
				...
				<div class="home_elections_posts"><?php foreach($posts as $election_post){
...
function add_recipient_to_list($recipient){
	$url = 'https://api.sendgrid.com/v3/';
$request =  $url.'contactdb/lists/9850158/recipients/'.$recipient; 

// Generate curl request
$ch = curl_init();
$headers = 
array("Content-Type: application/json",
"Authorization: Bearer SENDGRID_API_KEY_AQUI");
curl_setopt($ch, CURLOPT_POST, true);   
curl_setopt($ch, CURLOPT_URL, $request);
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
curl_setopt($ch, CURLOPT_TIMEOUT, 60);
...
// {
	...
	// $is_anticorrupcion = $single_schedule[17]=='1'?true:false;
	// $is_ciudades = $single_schedule[18]=='1'?true:false;
	// $is_competitividad = $single_schedule[19]=='1'?true:false;
	// $is_education = $single_schedule[20]=='1'?true:false;
	// $is_energia = $single_schedule[21]=='1'?true:false;
	// $is_gobierno = $single_schedule[22]=='1'?true:false;
	// $is_justicia = $single_schedule[23]=='1'?true:false;
	// $tipo = $single_schedule[24];
	// $areas = array();
